...
func listStoraceAccountOwnerImpl(cmd *cobra.Command, args []string) {
	...
	if err := testConnections(); err != nil {
		...
		log.Info("collecting azure storage account owners...")
		start := time.Now()
		subscriptions := listSubscriptions(ctx, azClient)
		stream := listStorageAccountOwners(ctx, azClient, listStorageAccounts(ctx, azClient, subscriptions))
...
func listStorageAccountOwners(ctx context.Context, client client.AzureClient, storageAccounts <-chan interface{}) <-chan interface{} {
	var (
	...
	go func() {
		...
		for result := range pipeline.OrDone(ctx.Done(), storageAccounts) {
			if storageAccount, ok := result.(AzureWrapper).Data.(models.StorageAccount); !ok {
				log.Error(fmt.Errorf("failed type assertion"), "unable to continue enumerating storage account owners", "result", result)
				return
			} else {
		...
		go func() {
			...
			for id := range stream {
				...
				for item := range client.ListRoleAssignmentsForResource(ctx, id.(string), "") {
					if item.Error != nil {
						log.Error(item.Error, "unable to continue processing owners for this storage account", "storageAccountId", id)
					} else {
