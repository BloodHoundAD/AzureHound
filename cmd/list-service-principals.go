...
func listServicePrincipalsCmdImpl(cmd *cobra.Command, args []string) {
	...
	start := time.Now()
	panicChan := panicChan()
	stream := listServicePrincipals(ctx, azClient, panicChan)
...
func listServicePrincipals(ctx context.Context, client client.AzureClient, panicChan chan error) <-chan interface{} {
